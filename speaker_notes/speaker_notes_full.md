...
The architecture is a graph of graphs. Outer structure is sequential — start, Intent, Observability, Planning, Infra Action, end — and each of those four agents is itself a small subgraph with its own internal reasoning steps. The deployment view on the right is intentionally simple: the specialist pods send prompts to a shared LLM backend and tool calls to a shared MCP server, both services send results back, and the container orchestrator returns updated cluster state after execution. On top of that, every agent satisfies the PARES capability contract: Perceive, Act, Reason, Evaluate, Sustain — the five capabilities that define what it means to be an agent in this framework rather than a prompt chain. The point is specialised but still debuggable: explicit request-response boundaries instead of one opaque monolith.
...
The simulator does two jobs at once. At runtime, it is the digital twin that the Planning agent uses inside ActSimCrit to validate a proposed tool-call batch before anything is committed. As an evaluation harness, it is the controlled environment where all three AgentEdge experiments run. The tier table on-slide names the three levels — far edge, near edge, cloud — with typical per-tier node ranges, CPU and memory, power envelopes, latency, and the sixty-percent low-power saving applied in each tier. The first three scenarios are now reduced to compact visual patterns: move first, move then sleep, or redistribute. The twelve API endpoints — deploy, migrate, scale, low-power, status, and seven others — are listed as chips, and the simulator is explicit about its stochastic timing, its 0.1-to-2-percent failure probability, and its real-time validation. The bottom line on-slide — *same infrastructure, same LLM, same tools, only architecture varies* — is the fairness guarantee that makes the next three experiments comparable.

### Slide 40 · Scenario 1 - Multi-agent vs single-agent

The first AgentEdge experiment is an architecture-versus-architecture comparison, and the left side of the slide shows all three arms side by side as diagrams so we can see *why* they differ, not just that they differ. AgentEdge is the graph of graphs — four specialist agents (intent, observe, plan, infra_action) routed sequentially, with a digital-twin callout on the plan node: plans are simulated and critiqued before execution. ReAct is the one-LLM think-act loop — a single model interleaves reasoning, tool calls, and observation, and only stops on a finish action; no specialization, no plan validation. LATS is the same single LLM but wrapped in Monte-Carlo tree search — the model generates, scores, selects, and back-propagates over a tree of reasoning steps; it is the strongest single-agent baseline in the literature. The right side fixes the measurement protocol: each scenario pre-defines all valid infrastructure end-states and a run counts as successful only when the final placement exactly matches one of them — no partial credit. Sixty runs per arm — twenty each on S1, S2, S3 — with Wilson-score confidence intervals and SHA-256 seeding for reproducibility. Base LLM is `qwen3-235B`, the tool catalogue is the same twelve endpoints, the simulator state is identical, so only the architecture varies. If the same LLM, same tools, and same tasks yield different success rates, the gap cannot be explained by model capability — it must come from the architecture itself, and that is exactly what this experiment is designed to isolate.
...
The ablation turns the digital twin on and off and holds everything else constant. The diagram on the left shows the full AgentEdge: the three planning phases — strategic plan, state analysis, action select — are collapsed into a single *Plan* unit, then the P4 Sim-plus-Critic block validates the plan against the twin, and P5 asks whether the intent is met. Reject-and-retry is a *virtual* arc, bounded by the iteration and reject caps, and only a validated plan leaves the loop. On the right is the *WITHOUT* arm: identical planning stage, but the Sim-plus-Critic block is removed, so the plan is pushed straight to execution on real infra without pre-execution validation. Let me be precise about what "WITHOUT" means here, because this is a question that often comes up: on a failure the plan is refined *in place* by the Exec agent; no round-trip back to Planning. In other words, the twin's contribution is isolated as *pre-execution validation*, not as the existence of a retry mechanism at all. The Q&A anticipation on-slide is the related point: the twin does not change *whether* a plan can succeed, it changes *how many physical retries* it takes. We report both success rate and API-call count, so failure-shifting cannot hide behind either metric alone.
...
On the **left card**, the blocker is *evaluation*: one orchestration prompt admits many valid plans — consolidate, shut a node, migrate a service — they can all satisfy the same intent. And on top of that, the same LLM run twice produces different plans. Multi-solution tasks plus LLM non-determinism means there is no shared score. The visual makes that concrete: one prompt fans out to three valid plans, then collects into a benchmark box with a literal question-mark inside. The direction I propose is explicit: orchestration-trace datasets, equivalence-class scoring, and confidence intervals across repeated runs — not string-equal ground truth.

On the **right card**, the blocker is *trust at decision time*: the agent shows operators only what it chose, not what it rejected, and nothing formally checks the output. The tree on the slide draws one chosen green branch and two dashed rejected branches, each labelled with the reason it was ruled out — would violate SLA, infeasible resource request. Next to it is a dashed constraint-checker box that today simply does not exist in most deployments. The direction is reasoning-trace standards so the operator sees the rejected alternatives, plus constraint-clamping with natural-language feedback so the checker can tell the LLM exactly which rule it broke, plus critic-of-critics verification built on ActSimCrit. Trust is won by showing the paths that did not make it, not by making the chosen one louder.
...
On the **left card**, *inference efficiency*. Each orchestration is a chain of LLM calls — observe, plan, revise, critique, actuate — and every call is seconds, not milliseconds. The timeline visual stacks those boxes along the real axis and drops a red dashed real-time-SLA line right through the critic step. The overflow bracket on the right of the axis flags the consequence: the SLA breaks and token cost compounds at scale. The direction I argue for is call-reduction via multi-step prompting and, more importantly, distilled orchestration-specific models right-sized per decision. That is the AERO lesson applied to the planner: the answer to latency is not a faster frontier model, it is a smaller, specialised one.

On the **right card**, *tool discovery*. This one is an empirical finding from the thesis itself. Tool-pick accuracy stays flat up to roughly twenty tools and then cliffs. The chart shows exactly that shape, with a highlighted cliff at thirty and an annotated AgentEdge point — three MCP servers, about forty tools — sitting right on the cliff. I also keep the counterintuitive remark the dissertation reports: removing an MCP server sometimes *improved* success. The direction is semantic tool retrieval, hierarchical catalogs, and autonomous read-only tool discovery, so the planner sees a focused relevant set rather than the full shelf every turn. Latency and tool surface are both context-engineering problems, not hardware ones.
