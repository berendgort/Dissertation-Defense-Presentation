...
At 6G scale that quickly becomes a three-hundred-plus-service problem, so the real barrier is not inference cost but retraining burden. Research question two therefore becomes, framed openly: how can one forecasting framework be designed to cover heterogeneous service workloads without per-service retraining?
...
The scalability experiment asks a practical question: does AgentEdge keep saving energy as the infrastructure grows? Same task, three infrastructure sizes, concrete and operational rather than schematic. The environment is the full three-tier edge-cloud: far-edge nodes draw ten to thirty-five watts each, near-edge forty to one-hundred-twenty, and cloud nodes one-hundred-eighty to four-hundred-fifty, which is why consolidation and power-down have real wattage at stake. Low-power mode drops draw by about sixty percent; power is sampled every five seconds, and each scale is run ten times, deterministically seeded, so what we report is a measured power window, not a back-of-the-envelope estimate. The start state at every scale is services spread across nodes with some nodes left idle, and those idle nodes still draw baseline power; that is the target the agent has to reclaim. The goal is consistent across all three scales: *consolidate services, power down idle nodes*. The three panels are *Scale A · Small* with 8 nodes, 6 services, 2 idle; *Scale B · Medium* with 18 nodes, 15 services, 3 idle, consolidation spans two racks; and *Scale C · Large* with 35 nodes, 30 services, 6 idle, cross-rack coordination stress. The legend is explicit: red blocks are service-bearing nodes, dashed white blocks are idle nodes that are candidates for power-down. The pass condition, stated on the footer strip, is sharper than before: the end-state must match the predefined target *and* the measured energy reduction must be greater than zero watts, tracked alongside response time and API-call count so that planning overhead dominating at thirty-five nodes would be visible, not hidden. That is the question the next slide answers.
...
On the **right card**, the blocker is *security*, and I want to be precise about the threat model. The concern is not external prompt injection: orchestration agents sit inside a protected perimeter where only authorized operators have access. The concern is what happens when an agent with Kubernetes API access makes a mistake. The right side is now framed cleanly as two open questions, which is all the committee needs to carry away. Q1, framed as a question rather than a direction: how do we catch a destructive action *before* it commits, flagging it inside the plan rather than after the fact. Q2, equally direct: where do we let it safely fail? A physical sandbox on real hardware, not just a simulator. Two questions, not two to-do lists. The V-JEPA 2 world-model direction on the left card now carries its canonical reference on the slide, Assran et al. 2025, so the committee can place it in context.
