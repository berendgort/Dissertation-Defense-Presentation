...
- `speaker_notes/`
  ...
  - `speaker_notes_pptx.md`: shorter presenter-note version for glanceable delivery.
  - `update_speaker_notes.py`: syncs `speaker_notes_full.md` into the embedded notes JSON in `defense_html/_template.html` and validates note count against the deck.
...
2. If notes changed, run `python3 speaker_notes/update_speaker_notes.py`.
3. Confirm the target slide rebuilds into `Defense.html`.
4. Thoroughly check in-browser that every slide block still fits: text overflow is the most common regression.
5. Check the slide for overflow, spacing, and readability.
6. Prefer fast iteration loops: rerun/reload quickly and visually verify each slide change before moving on.
7. Make sure slide numbering, ordering, and section color semantics still make sense.
